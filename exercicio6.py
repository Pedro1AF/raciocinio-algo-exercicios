...
#       coloquei com valor de nota minima de 4 por dó dos alunos que tiram 1 ponto
    ...
    elif numExercicio == 10:
       vetorTodos = [random.randint(0, 100) for i in range(5)]
       print(f"Vetor completo: {vetorTodos}")

       menorValor = min(vetorTodos)
       maiorValor = max(vetorTodos)

       somaNum = sum(vetorTodos)
       print(f"O valor da soma de todos estes valores: {somaNum}")
       mediaVetor = somaNum / len(vetorTodos)
       print(f"Valor da média deste vetor é: {mediaVetor}")
    elif numExercicio == 11:
       vetorFinal = [random.randint(0, 100) for i in range(5)]
       maiorValor = max(vetorFinal)
       menorValor = min(vetorFinal)
       for i in range(len(vetorFinal)):
           if vetorFinal[i] == maiorValor:
               print(f"O maior valor do vetor é {maiorValor} e sua posição atual é [{i}]")
           elif vetorFinal[i] == menorValor:
               print(f"O menor valor do vetor é {menorValor} e sua posição atual é [{i}]")
    else:
        print("Insira um valor de 1 a 11 por favor")
        pass